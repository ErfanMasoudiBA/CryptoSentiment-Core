        ...
        const promises = coins.map(async (coin) => {
          try {
            const response = await axios.get(
              `http://127.0.0.1:8000/api/stats?q=${encodeURIComponent(coin.name)}`
            );
            return {
              name: coin.name,
              symbol: coin.symbol,
      ...
      <main className="flex-1 p-8">
        <div className="max-w-7xl mx-auto space-y-6">
          <div>
            ...
            <p className="text-slate-400">Explore sentiment analysis for different cryptocurrencies</p>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {coinsData.map((coin) => {
                ...
                return (
                  <div
                    key={coin.name}
                    className="bg-slate-800/50 backdrop-blur-sm rounded-xl border border-slate-700/50 shadow-lg hover:shadow-xl transition-all hover:border-blue-500/50 p-6"
                  >
                    <div className="flex items-start justify-between mb-4">
                      <div>
                        <h3 className="text-xl font-bold text-white mb-1">{coin.name}</h3>
                    ...
                    <div className="grid grid-cols-3 gap-2 mb-4 text-xs">
                      <div className="text-center p-2 bg-green-500/10 rounded-lg border border-green-500/20">
                        <div className="text-green-400 font-bold">{coin.stats.positive}</div>
                        <div className="text-slate-400">Positive</div>
                      </div>
                      <div className="text-center p-2 bg-red-500/10 rounded-lg border border-red-500/20">
                        <div className="text-red-400 font-bold">{coin.stats.negative}</div>
                      ...
                      <div className="text-center p-2 bg-slate-500/10 rounded-lg border border-slate-500/20">
                        <div className="text-slate-400 font-bold">{coin.stats.neutral}</div>
                    ...
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg transition-colors shadow-md hover:shadow-lg"
                    >
